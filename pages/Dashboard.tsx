...
const KPICard: React.FC<{ 
  ...
  <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col gap-4 transition-colors duration-200">
    <div className="flex justify-between items-start">
      <div className={`${color} p-3 rounded-xl`}>
        {icon}
      </div>
      {trend && (
        <div className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full ${trend.type === 'up' ? 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 dark:text-emerald-400' : 'text-rose-600 bg-rose-50 dark:bg-rose-900/30 dark:text-rose-400'}`}>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Ringkasan Gudang</h1>
          <p className="text-slate-500 dark:text-slate-400">Pantau pergerakan stok barang secara real-time</p>
        </div>
        
        {/* Year Selector Logic */}
        <div className="bg-white dark:bg-slate-800 p-1 rounded-xl border border-slate-100 dark:border-slate-700 flex items-center shadow-sm transition-colors duration-200">
          {years.length <= 4 ? (
            years.map(y => (
              <button
                key={y}
                ...
                className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                  selectedYear === y ? 'bg-purple-600 text-white shadow-md' : 'text-slate-500 dark:text-slate-400 hover:text-purple-600 dark:hover:text-purple-400'
                }`}
              >
                {y}
              </button>
            ...
            <div className="relative inline-block">
              <select 
                ...
                className="appearance-none bg-transparent pl-4 pr-10 py-2 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-200 focus:outline-none cursor-pointer"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard 
          ...
          icon={<Package className="text-purple-600 dark:text-purple-400" />} 
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm transition-colors duration-200">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
                <span className="w-3 h-3 bg-slate-200 dark:bg-slate-600 rounded-sm"></span> Keluar
              </div>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', backgroundColor: 'var(--tw-colors-slate-800)', color: 'white' }}
                />
                <Bar 
                  dataKey="incoming" 
                  name="Masuk" 
